...
int rhsInterferenceWrapper(double x, const double y[], double dydt[],
                           void *params_ptr) {
...
Interference::Interference(Reaction &R, int i, int isign) : Reac(R) {
...
void Interference::makeSamples(std::vector<std::vector<double>> Ref_sample,
  ...
  for (int iRes = 0; iRes < 2; iRes++) {
    ...
    // std::cout << "MakeSamples: Exf[" << iRes << "] = " << Res[iRes]->getExf()
    // << std::endl;
  }
...
double Interference::calcBroad(double T) {
  ...
  for (int s = 0; s < NSamples; s++) {
    ...
    Rate_sample[s] =
        NumericalRate(T, E_sample, G_sample, erFrac, sign_sample[s], false);
...
double Interference::NumericalRate(double T, double E_sample[2],
                                   double G_sample[2][3], double erFrac[2][3],
                                   double sign_sample, bool writeIntegrand) {
  ...
  for (int iRes = 0; iRes < 2; iRes++) {
    ...
    //"\n"; std::cout << "E_sample = " << E_sample[iRes] << ", L[iRes][] = " <<
    // L[iRes][0] << " " << 	L[iRes][1] << " " << L[iRes][2] << "\n";
    ...
    if (E_cm[iRes] > 0.0 && E_sample[iRes] < 0.0) {
      ...
      SampledNegCount++;
      G_sample[iRes][0] =
          mue * gsl_pow_2(R) * G_sample[iRes][0] /
          (2.0 * 41.80161396 *
           PenFactor(E_cm[iRes], L[iRes][0], M0, M1, Z0, Z1, R));
      // std::cout << "Positive resonance went negative!\n";
      // std::cout << "E_cm = " << E_cm << "E_sample = " << E_sample[iRes] <<
      // "G[0] = " << 	G[0]	<< " G_sample = " << G_sample[iRes][0] <<
      // std::endl;
    ...
    if (Reac.getGamma_index() == 2) {
      ...
      Pr_exit[iRes] =
          PenFactor(E_sample[iRes] + Reac.Q - Reac.Qexit - Exf[iRes],
                    L[iRes][1], M0 + M1 - M2, M2, Z0 + Z1 - Z2, Z2, R);
      ...
      Pr_exit[iRes] =
          PenFactor(E_sample[iRes] + Reac.Q - Reac.Qexit, L[iRes][2],
  ...
  if (status != 0) {
    ...
    gsl_odeiv2_control *c = gsl_odeiv2_control_y_new(1e-100, 1.0e-6);
    ...
    while (x < x1) {
      ...
      if (x < E_sample[0] &&
          (x + h) > (E_sample[0] - (3.0 * (G_sample[0][0] + G_sample[0][1] +
                                           G_sample[0][2]))))
        smallstep = true;
      if (x < E_sample[1] &&
          (x + h) > (E_sample[1] - (3.0 * (G_sample[1][0] + G_sample[1][1] +
                                           G_sample[1][2]))))
        smallstep = true;
      ...
      int status = gsl_odeiv2_evolve_apply(e, c, s, &sys, &x, x1, &h, y);
...
double Interference::Integrand(double x, void *params) {
  ...
  double Scale[3];
  double RatePart[3];    // Stores the rate part for each resonance and the
                         // interference term
  ...
  for (int iRes = 0; iRes < 2; iRes++) {
    ...
    if (G1[iRes] > 0.0) {
      if (Reac.getGamma_index() == 1) {
        ...
        Scale[1] =
            pow((Reac.Q + x - Exf[iRes]) / (Reac.Q + Er[iRes] - Exf[iRes]),
                (2. * L[iRes][1] + 1.0));
      } else {
        E_exit = Reac.Q + x - Reac.Qexit - Exf[iRes];
        if (E_exit > 0.0) {
          P_exit = PenFactor(E_exit, L[iRes][1], M0 + M1 - M2, M2, Z0 + Z1 - Z2,
                             Z2, R);
    ...
    if (G2[iRes] > 0.0) {
      if (Reac.getGamma_index() == 2) {
        Scale[2] =
            pow((Reac.Q + x) / (Reac.Q + Er[iRes]), (2. * L[iRes][2] + 1.0));
      } else {
        E_exit = Reac.Q + x - Reac.Qexit;
        if (E_exit > 0.0) {
          P_exit = PenFactor(E_exit, L[iRes][2], M0 + M1 - M2, M2, Z0 + Z1 - Z2,
                             Z2, R);
    ...
    /*		// TODO from here!
         ...
         if (i == 2)
         Scale[i] = pow((Reac.Q + x) / (Reac.Q + Er[iRes]), (2. * L[iRes][i]
       + 1.0)); } else { if (i == 1) E_exit = Reac.Q + x - Reac.Qexit -
       Exf[iRes]; if (i == 2) E_exit = Reac.Q + x - Reac.Qexit; if (E_exit >
       0.0) { P_exit = PenFactor(E_exit, L[iRes][i], M0 + M1 - M2, M2, Z0 + Z1 -
       Z2, Z2, R); Scale[i] = P_exit / Pr_exit[iRes]; } else { Scale[i] = 0.0;
         }
         }
         } else {
         Scale[i] = 1;
    ...
    /*
      std::cout << iRes << "   " << x << " Scale[0]=" << Scale[0] << "
      Scale[1]=" << Scale[1]
    ...
    double S2 = gsl_pow_2(Er[iRes] - x) +
                0.25 * gsl_pow_2(G0[iRes] * Scale[0] + G1[iRes] * Scale[1] +
                                 G2[iRes] * Scale[2]);
    ...
    double integrand =
        S1 * S3 / S2; //*3.7318e10*(pow(mue,-0.5)*pow(Temp,-1.5));
    ...
    delta[iRes] =
        atan((G0[iRes] * Scale[0] + G1[iRes] * Scale[1] + G2[iRes] * Scale[2]) /
             (2.0 * (x - Er[iRes])));
    if (delta[iRes] < 0.0)
      delta[iRes] += M_PI;
    ...
    singular_point =
        (diff <= (std::numeric_limits<double>::epsilon() * Er[iRes]));
  }
  ...
  SFactorPart[2] =
      sign * 2.0 * sqrt(SFactorPart[0] * SFactorPart[1]) * cos(del);
  ...
  if (writeIntegrand) {
    integrandfile << std::scientific << std::setprecision(9) << x << " "
                  << totalIntegrand << " " << totalSFactor << std::endl;
  }
...
int Interference::rhs(double x, const double y[], double dydx[], void *params) {
  ...
  for (int iRes = 0; iRes < 2; iRes++) {
    ...
    /*
      ...
      Scale[i] = pow((Reac.Q + x) / (Reac.Q + Er[iRes]), (2. * L[iRes][i]
      + 1.0)); } else { if (i == 1) E_exit = Reac.Q + x - Reac.Qexit -
      Exf[iRes]; if (i == 2) E_exit = Reac.Q + x - Reac.Qexit; if (E_exit > 0.0)
      { P_exit = PenFactor(E_exit, L[iRes][i], M0 + M1 - M2, M2, Z0 + Z1 - Z2,
      Z2, R); Scale[i] = P_exit / Pr_exit[iRes]; } else { Scale[i] = 0.0;
      }
      }
      } else {
      Scale[i] = 1;
    ...
    // Check for singular points
    // double diff = fabs(x - Er[iRes]);
    // singular_point = (diff <= (std::numeric_limits<double>::epsilon() *
    // Er[iRes]));
  }
...
double Interference::getSFactor(double E, int sign, int samp) {
  ...
  for (int iRes = 0; iRes < 2; iRes++) {
    ...
    if (Reac.getGamma_index() == 2) {
      // if exit particle is observed decay, take final excitation into account
      Pr_exit = PenFactor(E_cm_samp + Reac.Q - Reac.Qexit - Exf, L[iRes][1],
                          M0 + M1 - M2, M2, Z0 + Z1 - Z2, Z2, R);
    } else if (Reac.getGamma_index() == 1 && NChannels[iRes] == 3) {
      // ignore spectator final excitation if it is spectator
      //			std::cout << E << " E_cm_samp + Reac.Q -
      // Reac.Qexit = " << E_cm_samp + Reac.Q - Reac.Qexit
      //								<<
      // std::endl;
      Pr_exit = PenFactor(E_cm_samp + Reac.Q - Reac.Qexit, L[iRes][2],
                          M0 + M1 - M2, M2, Z0 + Z1 - Z2, Z2, R);
    }
    // std::cout << iRes << "   " << E << " Pr=" << Pr << " Pr_exit=" << Pr_exit
    // << std::endl; std::cout << iRes << "   " << E << " Scale[0]=" << Scale[0]
    // << std::endl; 		std::cout << Reac.getGamma_index() << std::endl;
    //  Exit scale
    if (G1 > 0.0) {
      if (Reac.getGamma_index() == 1) {
        // std::cout << Exf << " " << L[iRes][1] << std::endl;
        Scale[1] = pow((Reac.Q + E - Exf) / (Reac.Q + E_cm_samp - Exf),
                       (2. * L[iRes][1] + 1.0));
      } else {
    ...
    if (G2 > 0.0) {
      if (Reac.getGamma_index() == 2) {
        Scale[2] =
            pow((Reac.Q + E) / (Reac.Q + E_cm_samp), (2. * L[iRes][2] + 1.0));
      } else {
    ...
    /*
      ...
      if (i == 1)
      Scale[i] =
      pow((Reac.Q + E - Exf[iRes]) / (Reac.Q + E_cm_samp - Exf[iRes]), (2. *
      L[iRes][i] + 1.0)); if (i == 2) Scale[i] = pow((Reac.Q + E) / (Reac.Q +
      E_cm_samp), (2. * L[iRes][i] + 1.0)); } else { if (i == 1) E_exit = Reac.Q
      + E - Reac.Qexit - Exf[iRes]; if (i == 2) E_exit = Reac.Q + E -
      Reac.Qexit; if (E_exit > 0.0) { P_exit = PenFactor(E_exit, L[iRes][i], M0
      + M1 - M2, M2, Z0 + Z1 - Z2, Z2, R); Scale[i] = P_exit / Pr_exit; } else {
      Scale[i] = 0.0;
      }
      }
      } else {
    ...
    /*
      std::cout << iRes << "   " << E << " Scale[0]=" << Scale[0] << "
      Scale[1]=" << Scale[1]
      << " Scale[2]=" << Scale[2] << std::endl;
    ...
    SFactorPart[iRes] =
        S1 * S2 / S3; //*3.7318e10*(pow(mue,-0.5)*pow(Temp,-1.5));
    delta[iRes] = atan((G0 * Scale[0] + G1 * Scale[1] + G2 * Scale[2]) /
                       (2.0 * (E - E_cm_samp)));
    if (delta[iRes] < 0.0)
      delta[iRes] += M_PI;
  }
