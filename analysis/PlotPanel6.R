...
lognorm <- function(x){
    (1/(sigma*sqrt(2*3.142))) * exp(-((log(x)-mu)^2)/(2*sigma^2))
}
...
##	 xlab="",ylab="",cex.axis=1.3,yaxs="i")
    ...
    ##  s <-  sd(log(samples/median(samples)))
    ##  h <- dnorm(hist[,1],mean=m,sd=s)
    ##  
    ##  ## The normalisation
    ##  hh <- h/(hist[,3]/lognorm_norm())
    ##  n <- median(hh[hist[,3]/lognorm_norm()>0.3 & !is.infinite(hh)])
    ##  ##n <- 1.1*max(h)
...
text(xpos,ypos,expression(log[10](N[A]*group(langle,sigma*v,rangle))),cex=2)
